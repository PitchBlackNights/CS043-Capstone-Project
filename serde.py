...
def validate_data(data: Any) -> None:
    ...
    if data["type"] not in [0, 1, 2]:
        queued_exceptions += f"`data[\"type\"]` has a value of '{data['type']}'. Expected a value of ['1', '2']!\n"  # Value of '0' means it's ungenerated, which we check for later
    if data["difficulty"] not in [0, 1, 2, 3]:
        queued_exceptions += f"`data[\"difficulty\"]` has a value of '{data['difficulty']}'. Expected a value of ['0', '1', '2', '3']!\n"
            ...
            if len(row) != 9:  # type: ignore
                ...
                for col_index, col in enumerate(row):  # type: ignore
                    if col not in [" ", "1", "2", "3", "4", "5", "6", "7", "8", "9"]:
                        queued_exceptions += f"`data[\"board\"][{row_index}][{col_index}]` has a value of '{col}'. Expected a numerical string!\n"  # type: ignore
    if data["type"] == 0:
...
def deserialize(data_str: str) -> Board:
    ...
    board.id = data["id"]
    Board.last_seed = max(int(board.id), Board.last_seed)
    board.type = Board.Type(data["type"])
    board.difficulty = Board.Difficulty(data["difficulty"])
...
def serialize(board: Board) -> str:
    ...
    data: dict[str, str | int | list[list[str]]] = {
        "id": board.id,
        "type": int(board.type),
        "difficulty": int(board.difficulty),
        "board": board.board,
    }
